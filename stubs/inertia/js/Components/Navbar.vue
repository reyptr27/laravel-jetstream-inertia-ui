...
import { IdIcon, EnIcon } from '@/Components/Icons/Outline';
import {
    handleScroll,
    isDark,
    scrolling,
...
const handleFullScreen = () => {
    localStorage.setItem('isFullscreen', isFullscreen.value.toString());
    fullscreenChangeToast();
};

const getIsFullscreenFromLocalStorage = () => {
    const storedValue = localStorage.getItem('isFullscreen');
        if (storedValue !== null) {
        isFullscreen.value = storedValue === 'true';
    }
};

const fullscreenChangeToast = () => {
    infoToast({
        text: isFullscreen.value ? "Entered fullscreen mode." : "Exited fullscreen mode.",
    });
};

onMounted(() => {
    document.addEventListener('scroll', handleScroll);
    getIsFullscreenFromLocalStorage();
                ...
                <Dropdown align="right" width="48">
                    <template #trigger>
                        <button v-if="$page.props.jetstream.managesProfilePhotos" class="flex text-sm border-2 border-transparent rounded-md transition focus:outline-none focus:ring focus:ring-purple-500 focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-dark-eval-1">
                            <img class="h-8 w-8 rounded-md object-cover" :src="$page.props.auth.user.profile_photo_url" :alt="$page.props.auth.user.name" />
                        </button>
